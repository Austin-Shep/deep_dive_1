...
const calulateBirthday = bd => moment().diff(moment(bd), "days");

const profileCard = (user) => {
    let post = $("#profileDisp"),
        newDiv = $("<div>").append(
            $("<h2>")
                .text(`name: ${user.val().name}`)
                .attr("data-name", user.val().name),
            $("<h3>")
                .text(`email: ${user.val().email}`)
                .attr("data-email", user.val().email),
            $("<h3>")
                .text(`birthday: ${moment.unix(user.val().birthday).format("MMMM Do")}`)
                .attr("data-birthday", user.val().birthday),
            $("<p>", { "class": "delete", "id": user.key })
                .text("X")
        )
